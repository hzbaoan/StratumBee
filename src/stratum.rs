...
use crate::bitcoin_encoding::parse_u32_be_hex;
...
#[derive(Clone)]
pub struct MinerLatencyRegistry {
    inner: Arc<RwLock<HashMap<String, SessionLatencyHandle>>>,
}

impl Default for MinerLatencyRegistry {
    fn default() -> Self {
        Self {
...
impl MinerLatencyRegistry {
    pub fn new() -> Self {
        Self::default()
    }

    async fn register(&self, worker: String, handle: SessionLatencyHandle) {
        self.inner.write().await.insert(worker, handle);
    }

    async fn unregister(&self, worker: &str) {
        self.inner.write().await.remove(worker);
    }

    pub async fn sample_all(&self) -> Vec<MinerLatencySample> {
        let handles = {
            let guard = self.inner.read().await;
            guard
                .iter()
                .map(|(worker, handle)| (worker.clone(), handle.clone()))
                .collect::<Vec<_>>()
        };

        let mut results = Vec::with_capacity(handles.len());
        for (worker, handle) in handles {
            results.push(handle.sample(worker).await);
        }
...
#[derive(Clone)]
struct SessionLatencyHandle {
    tx: mpsc::Sender<oneshot::Sender<ProbeResult>>,
}

impl SessionLatencyHandle {
    async fn sample(&self, worker: String) -> MinerLatencySample {
        let sampled_at = Utc::now();
        let (reply_tx, reply_rx) = oneshot::channel();
        if self.tx.send(reply_tx).await.is_err() {
            return MinerLatencySample {
                worker,
                latency_ms: None,
                sampled_at,
                peer_addr: None,
        ...
        match reply_rx.await {
            ...
            Err(_) => MinerLatencySample {
                worker,
                latency_ms: None,
                sampled_at,
                peer_addr: None,
                error: Some("latency probe dropped".to_string()),
            },
        }
    }
}
...
impl StratumServer {
    ...
    async fn handle_client(&self, stream: TcpStream, addr: SocketAddr) -> anyhow::Result<()> {
        ...
        let reader_capacity = self.config.max_line_bytes.min(8 * 1024);
        let mut reader = BufReader::with_capacity(reader_capacity, reader);

        let (tx, mut rx) = mpsc::channel::<String>(64);
        let (probe_tx, mut probe_rx) = mpsc::channel::<oneshot::Sender<ProbeResult>>(8);
        let session_latency_handle = SessionLatencyHandle { tx: probe_tx };
            ...
            loop {
                tokio::select! {
                    ...
                    maybe_probe = probe_rx.recv() => {
                        let Some(reply_tx) = maybe_probe else {
                            continue;
                        };
                        let _ = reply_tx.send(probe_result_from_writer(&writer));
                    }
                }
            }
        ...
        let vardiff_task = tokio::spawn(async move {
            ...
            loop {
                ...
                if let Some((msg, post_send)) = notify_msg {
                    ...
                    if !trace.miner.is_empty() {
                        vardiff_metrics
                            .record_miner_seen(&trace.miner, trace.difficulty, None, None, None)
                            .await;
                    }
                }
            }
        ...
        let run_result: anyhow::Result<()> = async move {
            ...
            loop {
                let read_result = tokio::time::timeout(
                    ...
                    read_limited_line(&mut reader, &mut buf, self.config.max_line_bytes),
                )
                .await;
                ...
                let n = match read_result {
                    ...
                    Ok(Ok(LimitedLineRead::Eof)) => break,
                    Ok(Ok(LimitedLineRead::Line(n))) => n,
                    Ok(Ok(LimitedLineRead::TooLong { observed_at_least })) => {
                        warn!(
                            "oversized Stratum line from {addr}: at least {} bytes > {}",
                            observed_at_least,
                    ...
                    "mining.authorize" => {
                        ...
                        if authorized {
                            ...
                            self.latency_registry
                                .register(worker, session_latency_handle.clone())
                                .await;
                        }
                    }
                    "mining.get_version" => {
                    ...
                    "mining.ping" => {
                        ...
                        if sent {
                            if let Some((worker, difficulty)) = heartbeat {
                                self.metrics
                                    .record_miner_seen(&worker, difficulty, None, None, None)
                                    .await;
                            }
                        }
                    }
                    "mining.suggest_difficulty" => {
                        self.handle_suggest_difficulty(&request, &session_state, &tx)
                            .await?;
                    }
                    "mining.submit" => {
                        self.handle_submit(&request, &session_state, &tx).await?;
                    }
                    "mining.configure" => {
                        ...
                        let configured_mask = {
                            let current_job = self.template_engine.current_job().await;
                            let current_vbrequired = if current_job.ready {
                                current_job.vbrequired
                            } else {
                                0
                            };
                            parsed_mask & allowed_mask & !current_vbrequired
                        };

                        {
                            let mut guard = session_state.lock().await;
                            guard.version_rolling_enabled = version_rolling_requested;
        ...
        if !worker.is_empty() {
            self.latency_registry.unregister(&worker).await;
        }
    ...
    async fn handle_suggest_difficulty(
        ...
        let (diff_msg, notify_msg) = {
            ...
            let target = suggested_initial_difficulty(
                suggested,
                self.config.start_difficulty,
                self.config.min_difficulty,
                self.config.max_difficulty,
    ...
    async fn handle_authorize(&self, request: &StratumRequest) -> bool {
    ...
    async fn handle_submit(
            ...
            None => {
                ...
                if !session_worker.is_empty() {
                    self.metrics
                        .record_stale(
                            &session_worker,
                            stale_reason.unwrap_or(StaleReason::Expired),
                        )
                        .await;
                }
                return Ok(());
        ...
        let version_mask = session_job.version_mask;
        ...
        debug_assert_eq!(version_mask, negotiated_version_mask & !job.vbrequired);
        ...
        let ack_error = if accepted_for_miner {
        ...
        let _ = tx
            .send(
                json!({"id": request.id.clone(), "result": accepted_for_miner, "error": ack_error})
                    .to_string(),
            )
            .await;
            ...
            if session_job.is_stale_block.load(Ordering::Acquire) {
                ...
                self.metrics
                    .upsert_block(BlockEvent {
                        ...
                        reward_status: "pending_submission".to_string(),
                        created_at: now,
                    })
                    .await;
                ...
                tokio::spawn(async move {
                    ...
                    let reward_status = if status == "submitted" || status == "duplicate" {
                    ...
                    metrics
                        .upsert_block(BlockEvent {
                            ...
                            confirmations: None,
                            in_active_chain: None,
                            reward_status: reward_status.to_string(),
                            created_at: Utc::now(),
                        })
                        .await;
                });
...
fn probe_result_from_writer(writer: &OwnedWriteHalf) -> ProbeResult {
    let peer_addr = writer.peer_addr().ok().map(|addr| addr.to_string());
    match tcp_latency_ms_from_stream(writer.as_ref()) {
...
fn tcp_latency_ms_from_stream(stream: &tokio::net::TcpStream) -> Result<f64, String> {
    use std::mem::{size_of, zeroed};
    use std::os::fd::AsRawFd;

    let fd = stream.as_raw_fd();
    let mut info = unsafe { zeroed::<libc::tcp_info>() };
    ...
    if info.tcpi_rtt == 0 {
        return Err("tcp rtt unavailable".to_string());
    }
    Ok(info.tcpi_rtt as f64 / 1000.0)
}

#[cfg(not(any(target_os = "linux", target_os = "android")))]
fn tcp_latency_ms_from_stream(_stream: &tokio::net::TcpStream) -> Result<f64, String> {
    Err("tcp rtt is only implemented on Linux/Android".to_string())
}
...
impl SessionState {
    ...
    fn mark_jobs_stale_block(&mut self) {
        ...
        self.jobs
            .retain(|job| !job.is_stale_block.load(Ordering::Relaxed));
    }
    ...
    fn push_job(
        ...
        let version_mask = self.version_mask & !job.vbrequired;
    ...
    fn find_job(&self, job_id: &str) -> Option<Arc<SessionJob>> {
        ...
        self.jobs
            ...
            .rev()
            .find(|entry| {
                entry.session_job_id == job_id && !entry.is_stale_block.load(Ordering::Acquire)
            })
            .cloned()
    }
}
...
fn build_notify_dispatch(
    ...
    if session_job.version_rolling_enabled {
        messages.push(build_set_version_mask(session_job.version_mask));
    }
    messages.push(build_notify_with_ntime(
...
async fn send_notify(
    ...
    let NotifyDispatch { messages, trace } = dispatch;
...
fn suggested_initial_difficulty(
...
fn build_set_version_mask(mask: u32) -> String {
    json!({
        "id": null,
...
enum LimitedLineRead {
    Eof,
    Line(usize),
    TooLong { observed_at_least: usize },
}
...
async fn read_limited_line<R: AsyncBufRead + Unpin>(
    ...
    loop {
        ...
        if let Some(newline_pos) = available.iter().position(|byte| *byte == b'\n') {
            ...
            reader.consume(take);
            return Ok(LimitedLineRead::Line(out.len()));
        }

        let chunk_len = available.len();
        let observed = out.len().saturating_add(chunk_len);
        if observed > max_bytes {
...
mod tests {
    ...
    #[tokio::test]
    async fn limited_line_reader_accepts_line_at_limit() {
        let input = b"abc\n";
        let mut reader = BufReader::new(&input[..]);
        let mut out = Vec::new();

        let result = read_limited_line(&mut reader, &mut out, input.len())
            .await
            .unwrap();

        assert_eq!(result, LimitedLineRead::Line(input.len()));
        assert_eq!(out.as_slice(), input);
    }

    #[tokio::test]
    async fn limited_line_reader_rejects_before_buffer_exceeds_limit() {
        let input = b"abcdef";
        let mut reader = BufReader::with_capacity(2, &input[..]);
        let mut out = Vec::new();

        let result = read_limited_line(&mut reader, &mut out, 4).await.unwrap();

        assert!(matches!(result, LimitedLineRead::TooLong { .. }));
        assert!(out.len() <= 4);
    }
    ...
    #[test]
    fn suggested_difficulty_only_replaces_start_when_higher() {
        assert_eq!(
            suggested_initial_difficulty(Some(32_768.0), 16_384.0, 1_024.0, 65_536.0),
            32_768.0
        );
        assert_eq!(
            suggested_initial_difficulty(Some(2_000.0), 16_384.0, 1_024.0, 65_536.0),
            16_384.0
        );
        assert_eq!(
            suggested_initial_difficulty(Some(1_000_000.0), 16_384.0, 1_024.0, 65_536.0),
            65_536.0
        );
        assert_eq!(
            suggested_initial_difficulty(Some(f64::NAN), 16_384.0, 1_024.0, 65_536.0),
            16_384.0
        );
    }

    #[test]
    fn push_job_shrinks_version_mask_by_job_vbrequired() {
        let mut state = SessionState::new(
            "en1".to_string(),
            vec![1, 2, 3, 4],
            1.0,
        ...
        job.vbrequired = 0x00002000;

        let session_job = state.push_job(Arc::new(job), 1.0, &[], None);

        assert_eq!(session_job.version_mask, 0x1fffc000);
    }

    #[test]
    fn notify_dispatch_sends_version_mask_before_notify() {
        let mut job = JobTemplate::empty();
        job.ready = true;
        job.job_id = "job-1".to_string();
        job.prevhash = "00".repeat(32);
        job.coinbase1 = "aa".to_string();
        job.coinbase2 = "bb".to_string();
        job.version = "20000000".to_string();
        job.nbits = "1d00ffff".to_string();
        job.ntime = "12345678".to_string();
        ...
        let session_job = SessionJob {
            ...
            notify_ntime: "12345678".to_string(),
            version_rolling_enabled: true,
            version_mask: 0x1fffc000,
            custom_coinbase2_hex: None,
            custom_coinbase2_bytes: None,
            is_stale_block: AtomicBool::new(false),
        };

        let dispatch = build_notify_dispatch("miner-1", 1.0, &session_job, true, "test");

        assert_eq!(dispatch.messages.len(), 2);
        assert!(dispatch.messages[0].contains("mining.set_version_mask"));
        assert!(dispatch.messages[0].contains("1fffc000"));
        assert!(dispatch.messages[1].contains("mining.notify"));
    }
    ...
    fn bootstrap_claim_is_single_shot_until_notify_send_commits() {
        ...
        state.note_notify_sent(NotifyPostSend::new("prevhash-1".to_string(), true, true));
    ...
    fn begin_authorized_session_clears_prior_job_state_and_resets_bootstrap_gate() {
        ...
        state.submitted_hashes.insert((1, 2, 3, [4; 16], 5, 6));
        state
            .submitted_hashes_order
            .push_back((1, 2, 3, [4; 16], 5, 6));
