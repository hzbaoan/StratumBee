...
use crate::bitcoin_encoding::encode_varint;
...
pub struct TemplateEngine {
    ...
    last_gbt_success_ms: Arc<AtomicU64>,
...
impl TemplateEngine {
    pub fn new(config: Config, rpc: RpcClient, counters: Arc<PoolCounters>) -> Self {
        ...
        Self {
            ...
            last_gbt_success_ms: Arc::new(AtomicU64::new(0)),
            counters,
        }
    }
    ...
    pub fn last_gbt_success_age_secs(&self) -> Option<u64> {
        let last = self.last_gbt_success_ms.load(Ordering::Acquire);
        if last == 0 {
            None
        } else {
            Some(Self::now_ms().saturating_sub(last) / 1_000)
        }
    }
    ...
    pub async fn handle_fast_block_announcement(
        ...
        if block_nbits != tip.nbits_u32 {
            warn!(
                "skip p2p fast block: nbits mismatch hash={} height={} block_nbits={:08x} expected={:08x}",
                block_hash_hex,
                block_height,
                block_nbits,
                tip.nbits_u32
            ...
            drop(permit);
            self.spawn_refresh_task(TemplateSource::P2pCmpctFast, "p2p fast nbits mismatch");
            return Ok(());
        }
        if !block_hash_meets_compact_target(block_hash_hex, block_nbits)? {
            warn!(
                "reject p2p fast block: invalid pow hash={} nbits={:08x}",
                block_hash_hex, block_nbits
            );
            drop(permit);
            return Ok(());
        }
        ...
        if !self.should_publish_fast_empty_job(next_height) {
            drop(permit);
            self.spawn_refresh_task(TemplateSource::P2pCmpctFast, "p2p fast guarded");
            return Ok(());
        }
        ...
        let key = format!(
            "fast:{}:{}:{}:{}",
            block_hash_hex, next_height, tip.nbits, tip.version_u32
        );
        ...
        info!(
            "new block source=p2p_fast height={} txs={} reward={} sat nbits={} clean_jobs=true",
            job.height,
        ...
        for (attempt, delay_ms) in submit_delays_ms.into_iter().enumerate() {
                ...
                .await
            {
                ...
                Ok(Some(reason)) => {
                    ...
                    return Ok(SubmitBlockOutcome::Rejected(reason));
                }
                Err(err) => {
                    tracing::error!(
    ...
    fn should_publish_fast_empty_job(&self, next_height: u64) -> bool {
        should_publish_fast_empty_job_for_network(self.config.network, next_height)
    }
    ...
    async fn longpoll_loop(&self) {
        ...
        loop {
            ...
            let params = gbt_request_params(self.config.network, longpoll_id.as_deref());

            match tokio::time::timeout(
                Duration::from_secs(LONGPOLL_TIMEOUT_SECS),
                self.rpc
                    .call_longpoll::<GbtResponse>("getblocktemplate", params),
            )
            .await
            {
                Ok(Ok(gbt)) => match self.apply_gbt_response(gbt, TemplateSource::Longpoll).await {
                    Ok(()) => self.note_gbt_success(),
                    Err(err) => warn!("longpoll apply failed: {err:?}"),
                },
                Ok(Err(err)) => {
                    warn!("longpoll GBT call failed: {err:?}; forcing one direct GBT refresh");
                    if let Err(refresh_err) = self
                        .refresh_template(TemplateSource::LongpollErrorFallback)
                        .await
                    {
                ...
                Err(_) => {
                    ...
                    if let Err(err) = self
                        .refresh_template(TemplateSource::LongpollTimeoutFallback)
                        .await
                    {
    ...
    async fn refresh_template(&self, source: TemplateSource) -> anyhow::Result<()> {
        ...
        loop {
            let gbt: GbtResponse = self
                ...
                .call(
                    "getblocktemplate",
                    gbt_request_params(self.config.network, None),
                )
                .await?;
            self.apply_gbt_response(gbt, source).await?;
            self.note_gbt_success();
    ...
    async fn apply_gbt_response(
        ...
        let current = self.current_job().await;
        if is_gbt_older_than_current(gbt.height, current.as_ref()) {
            warn!(
                "skipping stale GBT response source={} height={} current_height={}",
                source, gbt.height, current.height
            );
            self.counters.inc_notify_deduped();
            drop(permit);
            return Ok(());
        }
        let prev_prevhash_le = if current.ready {
            Some(current.prevhash_le.clone())
        } else {
            None
        };
        drop(current);
        validate_template_rules(&gbt.rules, self.config.network)?;

        let txid_root = compute_txid_partial_root(&gbt.transactions);
        let rules_key = gbt.rules.join(",");
        let key = format!(
            "{}:{}:{}:{}:{}:{}:{}:{}",
        ...
        let job = self.build_job(gbt, key.clone(), source, txid_root).await?;
        self.update_chain_tip_from_job(&job).await;
        let is_new_block = prev_prevhash_le
            .as_deref()
            .map_or(true, |p| p != job.prevhash_le);
        if is_new_block {
            info!(
                "new block source={} height={} txs={} reward={} sat nbits={} clean_jobs=true",
                job.template_source,
                job.height,
                job.transactions.len(),
                job.coinbase_value,
            ...
            info!(
                "template update source={} height={} txs={} reward={} sat nbits={} clean_jobs=false",
                job.template_source,
                job.height,
                job.transactions.len(),
                job.coinbase_value,
    ...
    fn note_gbt_success(&self) {
        self.last_gbt_success_ms
    ...
    async fn build_job(
        ...
        let (coinbase1, coinbase2, coinbase_tx, has_witness_commitment) = self
        ...
        let merkle_branches = merkle_branches_le
            .iter()
            .map(hex::encode)
            .collect::<Vec<_>>();
...
fn gbt_request_params(network: bitcoin::Network, longpollid: Option<&str>) -> serde_json::Value {
    let mut request = serde_json::Map::new();
    request.insert("rules".to_string(), json!(supported_gbt_rules(network)));
    if let Some(id) = longpollid.filter(|id| !id.is_empty()) {
        request.insert("longpollid".to_string(), json!(id));
    }
    serde_json::Value::Array(vec![serde_json::Value::Object(request)])
}

fn supported_gbt_rules(network: bitcoin::Network) -> Vec<&'static str> {
...
fn validate_template_rules(rules: &[String], network: bitcoin::Network) -> anyhow::Result<()> {
    for rule in rules {
        let Some(forced_rule) = rule.strip_prefix('!') else {
            continue;
        };
        if forced_rule.is_empty() {
            return Err(anyhow!("getblocktemplate returned empty forced rule"));
        }
        if !is_supported_forced_rule(forced_rule, network) {
            return Err(anyhow!("unsupported forced rule: {forced_rule}"));
        }
    }
    Ok(())
}

fn is_supported_forced_rule(rule: &str, network: bitcoin::Network) -> bool {
    matches!(rule, "segwit") || (rule == "signet" && network == bitcoin::Network::Signet)
}

fn is_gbt_older_than_current(gbt_height: u64, current: &JobTemplate) -> bool {
    current.ready && gbt_height < current.height
}

fn compact_target_to_le(nbits: u32) -> Option<[u8; 32]> {
    let exponent = (nbits >> 24) as usize;
    let mantissa = nbits & 0x007f_ffff;

    if (nbits & 0x0080_0000) != 0 || mantissa == 0 || exponent == 0 || exponent > 32 {
        return None;
    }

    let mut target_be = [0u8; 32];
    if exponent <= 3 {
        let reduced = mantissa >> (8 * (3 - exponent));
        for idx in 0..exponent {
            target_be[32 - exponent + idx] = ((reduced >> (8 * (exponent - 1 - idx))) & 0xff) as u8;
        }
    } else {
        let start = 32 - exponent;
        target_be[start] = ((mantissa >> 16) & 0xff) as u8;
        target_be[start + 1] = ((mantissa >> 8) & 0xff) as u8;
        target_be[start + 2] = (mantissa & 0xff) as u8;
    }
    target_be.reverse();
    Some(target_be)
}

fn block_hash_meets_compact_target(block_hash_hex: &str, nbits: u32) -> anyhow::Result<bool> {
    let mut hash_be = hex::decode(block_hash_hex).context("decode fast block hash")?;
    if hash_be.len() != 32 {
        return Err(anyhow!("fast block hash must be 32 bytes"));
    }
    hash_be.reverse();
    let hash_le: [u8; 32] = hash_be
        .try_into()
        .map_err(|_| anyhow!("fast block hash length must be 32"))?;
    let target_le =
        compact_target_to_le(nbits).ok_or_else(|| anyhow!("invalid compact target {nbits:08x}"))?;
    Ok(crate::share::leq_le256(&hash_le, &target_le))
}

fn should_publish_fast_empty_job_for_network(network: bitcoin::Network, next_height: u64) -> bool {
    network == bitcoin::Network::Bitcoin && next_height % 2016 != 0
}
...
fn encode_script_pushdata(data: &[u8], out: &mut Vec<u8>) -> anyhow::Result<()> {
    match data.len() {
        ...
        _ => {
            return Err(anyhow!("script pushdata too long: {} bytes", data.len()));
        }
    }
...
mod tests {
    ...
    #[test]
    fn coinbase_height_uses_bip34_script_number_pushes() {
        assert_eq!(encode_coinbase_height(0).unwrap(), vec![0x00]);
        assert_eq!(encode_coinbase_height(1).unwrap(), vec![0x51]);
        assert_eq!(encode_coinbase_height(16).unwrap(), vec![0x60]);
        assert_eq!(encode_coinbase_height(17).unwrap(), vec![0x01, 0x11]);
        assert_eq!(encode_coinbase_height(128).unwrap(), vec![0x02, 0x80, 0x00]);
    }

    #[test]
    fn stale_gbt_guard_only_rejects_lower_heights() {
        let empty = JobTemplate::empty();
        assert!(!is_gbt_older_than_current(99, &empty));

        let mut current = JobTemplate::empty();
        current.ready = true;
        current.height = 100;

        assert!(is_gbt_older_than_current(99, &current));
        assert!(!is_gbt_older_than_current(100, &current));
        assert!(!is_gbt_older_than_current(101, &current));
    }

    #[test]
    fn fast_empty_jobs_are_mainnet_only_and_skip_period_boundaries() {
        assert!(should_publish_fast_empty_job_for_network(
    ...
    #[test]
    fn supported_gbt_rules_include_signet_only_on_signet() {
        assert_eq!(
            supported_gbt_rules(bitcoin::Network::Bitcoin),
            vec!["segwit"]
        );
        assert_eq!(
            supported_gbt_rules(bitcoin::Network::Signet),
            vec!["segwit", "signet"]
        );
    }

    #[test]
    fn rejects_unknown_forced_rules() {
        let err = validate_template_rules(&["!unknown".to_string()], bitcoin::Network::Bitcoin)
            .expect_err("unknown forced rule should fail");
        assert!(format!("{err:#}").contains("unsupported forced rule"));
    }

    #[test]
    fn accepts_forced_signet_rule_only_on_signet() {
        validate_template_rules(&["!signet".to_string()], bitcoin::Network::Signet)
            .expect("signet forced rule should be accepted on signet");
        assert!(
            validate_template_rules(&["!signet".to_string()], bitcoin::Network::Bitcoin).is_err()
        );
    }

    #[test]
    fn compact_target_to_le_rejects_invalid_values() {
        assert!(compact_target_to_le(0).is_none());
        assert!(compact_target_to_le(0x1d80ffff).is_none());
    }

    #[test]
    fn block_hash_compact_target_check_matches_known_boundary() {
        let nbits = 0x1d00ffff;
        let target_le = compact_target_to_le(nbits).expect("valid compact target");
        let mut target_be = target_le;
        target_be.reverse();

        assert!(block_hash_meets_compact_target(&hex::encode(target_be), nbits).unwrap());

        let mut above_target = target_le;
        above_target[0] = above_target[0].saturating_add(1);
        let mut above_target_be = above_target;
        above_target_be.reverse();
        assert!(!block_hash_meets_compact_target(&hex::encode(above_target_be), nbits).unwrap());
    }
