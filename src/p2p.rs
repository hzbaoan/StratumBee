    ...
    async fn wait_for_startup_ready(&self, peer: &str) {
        loop {
            match self.probe_peer_ready(peer).await {
                Ok(remote_addr) => {
                    info!(
                        "p2p fast peer startup ready peer={} resolved={}",
                        peer, remote_addr
                    );
                    return;
                }
...
fn parse_block_announcement_from_header(header: &[u8]) -> anyhow::Result<BlockAnnouncement> {
    ...
    let block_nbits = u32::from_le_bytes(
        header[72..76]
            .try_into()
            .expect("block nbits field must be 4 bytes"),
    );
    ...
    *pos += 1;
    ...
    match prefix {
        ...
        0xfd => {
            ...
            *pos += 2;
            Ok(u16::from_le_bytes(bytes.try_into().expect("u16 varint must be 2 bytes")) as u64)
        }
        0xfe => {
            let bytes = payload
                .get(*pos..*pos + 4)
            ...
            *pos += 4;
            Ok(u32::from_le_bytes(bytes.try_into().expect("u32 varint must be 4 bytes")) as u64)
        }
        0xff => {
            let bytes = payload
                .get(*pos..*pos + 8)
...
mod tests {
    use super::{
        build_getheaders_payload, build_sendcmpct_payload, default_port, encode_network_address,
        network_magic, parse_cmpctblock_announcement, parse_headers_announcements,
        parse_inv_block_hashes, read_varint,
    };
    use crate::hash::double_sha256;
    use bitcoin::Network;
    ...
    fn encode_network_address_keeps_ipv6_octets() {
        ...
        let encoded = encode_network_address(IpAddr::V6(ip), 8333);
    ...
    #[test]
    fn regtest_uses_regtest_magic_and_default_port() {
    ...
    fn parse_cmpctblock_announcement_reads_prevhash_hash_and_time() {
        ...
        assert_eq!(announcement.block_nbits, 0);
    }
    ...
    fn parse_inv_block_hashes_filters_block_inventory() {
        ...
        crate::bitcoin_encoding::encode_varint(3, &mut payload);
    ...
    fn parse_headers_announcements_reads_prevhash_hash_and_time() {
        ...
        let mut payload = Vec::new();
        crate::bitcoin_encoding::encode_varint(1, &mut payload);
        ...
        assert_eq!(announcement.block_nbits, 0x1d00ffff);
    }
