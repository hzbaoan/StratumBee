...
const MAX_CHANNEL_JOBS: usize = 32;
...
struct ChannelJob {
    ...
    // SetTarget applies to future jobs and jobs received with empty min_ntime.
    updates_with_channel_target: bool,
...
impl Sv2Connection {
    ...
    async fn handle_open_extended_channel(
        ...
        let Some(extranonce_size) =
            fixed_sv2_extranonce_size(self.config.extranonce2_size, min_extranonce_size)
        else {
            self.metrics.counters.inc_sv2_channel_open_error();
            self.writer
                .send_message(
        ...
        let extranonce_prefix_len = self.config.extranonce1_size;
    ...
    async fn send_job_to_channel(
        ...
        let channel_job = ChannelJob {
            ...
            created_at: Utc::now(),
            updates_with_channel_target: clean,
        ...
        if clean {
            ...
            prune_extra_jobs(channel, MAX_CHANNEL_JOBS);
        }
    ...
    async fn maybe_retarget_channel(
        ...
        let Some((new_difficulty, new_target, worker, template)) =
            self.retarget_channel_state(channel_id, Some(share_difficulty), now)
        else {
            return Ok(());
        };

        self.send_channel_retarget(channel_id, new_difficulty, new_target, worker, template)
            .await
    }
    ...
    async fn send_channel_retarget(
        &mut self,
        channel_id: u32,
        new_difficulty: f64,
        new_target: [u8; 32],
        worker: String,
        template: Option<Arc<JobTemplate>>,
    ) -> anyhow::Result<()> {
        self.send_channel_target(channel_id, new_difficulty, new_target, worker)
            .await?;
        if let Some(template) = template {
            self.send_job_to_channel(channel_id, template, false)
                .await?;
        }
        Ok(())
    }
    ...
    fn retarget_channel_state(
        ...
        now: chrono::DateTime<Utc>,
    ) -> Option<(f64, [u8; 32], String, Option<Arc<JobTemplate>>)> {
        ...
        let current_template = channel
            .current_job_id
            .and_then(|job_id| channel.jobs.get(&job_id))
    ...
    async fn submit_block_candidate(
        ...
        tokio::spawn(async move {
                ...
                .await
            {
                ...
                Ok(SubmitBlockOutcome::Rejected(reason)) => ("rejected".to_string(), Some(reason)),
...
fn fixed_sv2_extranonce_size(configured_size: usize, requested_min_size: usize) -> Option<usize> {
    (requested_min_size <= configured_size).then_some(configured_size)
}
...
fn set_channel_difficulty(channel: &mut ChannelState, difficulty: f64) -> anyhow::Result<[u8; 32]> {
    ...
    for job in channel.jobs.values_mut() {
        if job.updates_with_channel_target {
...
fn prune_extra_jobs(channel: &mut ChannelState, max_jobs: usize) {
    if channel.jobs.len() <= max_jobs {
        return;
    }

    let mut job_ids = channel
        .jobs
        .iter()
        .map(|(job_id, job)| (*job_id, job.created_at.timestamp_millis()))
        .collect::<Vec<_>>();
    job_ids.sort_unstable_by_key(|(_, created_at_ms)| *created_at_ms);
    for (job_id, _) in job_ids {
        if channel.jobs.len() <= max_jobs {
            break;
        }
        if Some(job_id) != channel.current_job_id {
...
mod tests {
    ...
    fn channel_with_current_job(prevhash_le: &str) -> ChannelState {
        ...
        let share_target = share_target_le(1.0).unwrap();
        ...
        jobs.insert(
            ...
            ChannelJob {
                ...
                difficulty: 1.0,
                created_at: Utc::now(),
                updates_with_channel_target: false,
    ...
    #[test]
    fn sv2_extranonce2_size_matches_sv1_configuration() {
        assert_eq!(fixed_sv2_extranonce_size(8, 0), Some(8));
        assert_eq!(fixed_sv2_extranonce_size(8, 8), Some(8));
        assert_eq!(fixed_sv2_extranonce_size(8, 9), None);
    }
    ...
    #[test]
    fn sv2_channel_retarget_keeps_active_min_ntime_jobs_at_original_target() {
        let mut channel = channel_with_current_job("prevhash-a");
        let original_target = channel.jobs.get(&7).unwrap().share_target_le;
        ...
        assert_eq!(job.share_target_le, original_target);
        assert_ne!(job.share_target_le, target);
    }

    #[test]
    fn sv2_channel_retarget_updates_future_empty_min_ntime_jobs() {
        let mut channel = channel_with_current_job("prevhash-a");
        channel
            .jobs
            .get_mut(&7)
            .unwrap()
            .updates_with_channel_target = true;
