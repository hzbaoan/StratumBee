...
async fn run(cli: Cli) -> anyhow::Result<()> {
    ...
    let stratum = StratumServer::new(config.clone(), template_engine.clone(), metrics.clone());
    let stratum_handle = tokio::spawn(async move { stratum.run().await });

    let api_handle = if config.api_enabled {
        let api = ApiServer::new(config.clone(), metrics.clone(), rpc.clone(), template_engine);
