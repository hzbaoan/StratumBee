...
async fn run(cli: Cli) -> anyhow::Result<()> {
    ...
    info!(
        ...
        config.network, config.stratum_bind, config.stratum_port, config.api_bind, config.api_port
    );

    if config.sv2_enabled {
        sv2::log_settings(&config);
    }

    template_engine
        .start()
        .await
        .context("start template engine")?;

    let block_monitor = BlockMonitor::new(rpc.clone(), metrics.clone());
    ...
    let sv2_handle = if config.sv2_enabled {
        let sv2_config = config.clone();
        let sv2_template_engine = template_engine.clone();
        let sv2_metrics = metrics.clone();
        Some(tokio::spawn(async move {
            sv2::run(sv2_config, sv2_template_engine, sv2_metrics).await
        }))
    } else {
        None
    };
    ...
    match (api_handle, sv2_handle) {
        (Some(api_handle), Some(sv2_handle)) => {
            tokio::select! {
                res = stratum_handle => handle_task_result(res, "stratum")?,
                res = api_handle => handle_task_result(res, "api")?,
                res = sv2_handle => handle_task_result(res, "sv2")?,
...
fn handle_task_result(
    ...
    match res {
        ...
        Ok(Err(err)) => Err(err).with_context(|| format!("{name} exited with error")),
        Err(err) => Err(err).with_context(|| format!("{name} task join failed")),
    }
}
