...
Production note: V1 is the established production path. The SV2 Extended Channel path has been exercised in the included WSL regtest probe and uses the same solved-block archive plus `submitblock` path as V1. Public production exposure should still be gated by independent SRI client or proxy interoperability testing and host-level network controls.
...
- Solved SV2 candidates use the same local block archive and `submitblock` path as V1.
...
- A named volume mounted to `/opt/stratumbee/var`
